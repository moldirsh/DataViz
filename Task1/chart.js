async function buildPlot() {
    ...
    const yAccessorMax = (d) => d.temperatureHigh;
    ...
    bounded.append('text') // adds text on top of graph
        ...
        .text('HighTemperature (red line) and LowTemperature (blue line) by Date');

    const yLowScaler = d3.scaleLinear()
        .domain(d3.extent(data,yAccessorMin))
        .range([dimension.boundedHeight, 50]);

    const yHighScaler = d3.scaleLinear() // adds scaler for high temperature
    ...
    var x_axis = d3.axisBottom() // creates bottom horizontal axis
        .scale(xScaler);

    var y_axis = d3.axisLeft() // creates left vertical axis
        .scale(yLowScaler);
